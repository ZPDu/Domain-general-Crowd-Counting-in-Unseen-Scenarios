    ...
    def train(self, epoch, data_loaders, optimizer, print_freq=10, train_iters=400):
        ...
        for i in range(train_iters):
            ...
            for t in range(source_count):  # 0 1
                ...
                pred_mtr, sim_orth_loss = inner_model.train_forward(inputs, trainid)
                loss_mtr = self.criterion(pred_mtr, targets) + sim_orth_loss
                ...
                # meta test begins
                pred_mte, sim_loss  = inner_model.train_forward(testInputs, testid)

                loss_mte = self.criterion(pred_mte, testMaps) + sim_loss
